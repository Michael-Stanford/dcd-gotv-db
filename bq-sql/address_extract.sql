...
(address_id, city, zip, zip4, street_number, street_pre_dir, street_name, street_type, street_post_dir, melissa_data_results, address_type, 
 us_cong_district, state_house_district, state_senate_district, precinct_name, longitude, latitude, full_address, create_date, modified_date, query_date)
with xx as (
  select upper(trim(a.city)) as city, upper(trim(a.zip)) as zip, upper(a.zip4) as zip4, upper(trim(a.street_pre_dir)) as street_pre_dir, 
       ...
       cast(address_id as int64) as i_id, regexp_replace(regexp_replace(trim(street_number), r" .*$", ""), r"[A-Z]","") as street_number
...
select a.i_id as address_id, a.city, a.zip, a.zip4, a.street_number, a.street_pre_dir, a.street_name, a.street_type, a.street_post_dir, 
