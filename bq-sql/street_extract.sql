...
drop table if exists `demstxdallascp.sbx_farrarb.street_extract`;
create table if not exists `demstxdallascp.sbx_farrarb.street_extract`
(
  street_id INT64,
...
insert into `demstxdallascp.sbx_farrarb.street_extract` 
...
select street_pre_dir, street_name, street_type, street_post_dir, city, zip, 
   ...
   (select max(query_date) from `demstxdallascp.sbx_farrarb.address_extract`) as query_date
from xx a
--limit 100
;

-- Set Street ID
update `demstxdallascp.sbx_farrarb.street_extract` s
set s.street_id = 
   (select min(address_geo_id) from `demstxdallascp.sbx_farrarb.address_extract` a where a.full_street_name = s.full_street_name)
where s.full_street_name is not null
;

-- Update Address Extract with Street ID
update `demstxdallascp.sbx_farrarb.address_extract` a
set street_id = 
   (select distinct street_id from `demstxdallascp.sbx_farrarb.street_extract` s where a.full_street_name = s.full_street_name)
